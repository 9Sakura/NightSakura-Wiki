...
详细说明可前往[经济制度](Ns_Server/eco.md)查看

| 命令 | 命令说明 | 命令备注 |
| - | - | - |
| `/hm`| 打开桜云商城 ||
| `/hm <价格>` | 向桜云商城上架物品 | 税率8% |
| `/hshop create <sell/buy>` | 创建`出售/收购`木牌商店 | 需要对准空木牌 |
| `/hshop create lotto <单价>` | 设置乐透木牌商店 | 需要对准空木牌 |
| `/hshop sell <单价>` | 向自己的木牌商店上架物品 | 需要对准自己的出售型木牌 |
| `/hshop buy <单价>` | 向自己的木牌商店收购物品 | 需要对准自己的收购型木牌 |
| `/hchest req` | 设置收购商店的存储箱 | 需要对准箱子 |
| `/hchest lotto` | 将箱子内容设置为乐透商店奖池 | 需要对准箱子 |
| `/hchest remove` | 移除这个箱子的使用 | 需要对准箱子 |
| `/hframe set` | 设置展示框商店 | 展示框每30秒随机展示出售商店的物品 |
| `/hframe remove` | 移除展示框商店 ||
| `/hauc <价格> <进步价> [保留价]` | 对主手中的物品发起拍卖 | 交易成功买家支付**10%**手续费 |
| `/hreq <物品名/HAND> <价格> <数量>` | 征购物品 ||
| `/hsellto <玩家名> <总价>` | 向指定玩家发其直送订单 ||
| `/hpay <账单ID>` | 支付指定的账单 ||
| `/hcancel <账单ID>` | 取消指定账单 ||
| `/hstorage` | 打开临时仓储 ||
| `/hsearch <关键词>` | 搜索HEH商城的内容 ||
...
| `/nu exhibition inv` | 将光标所指展示框设为隐形状态 | 仅可于生存世界与生存下界使用 |
| `/nu exhibition uninv` | 取消光标所指展示框的隐形状态 | 仅可于生存世界与生存下界使用 |
...
| `/nu expcap store <经验值/ALL>` | 将经验储存在溶剂内 | 胶囊载体为**附魔之瓶** |
| `/nu expcap restore <经验值/ALL>` | 将经验从溶剂中取出 ||
